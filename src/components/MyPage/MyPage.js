    ...
    return (
        <div>
            ...
            <div class="mypageform">
                ...
                <h2> My Page </h2>
                <li class="useridform">
                    <h2 class="user_id">{User.email}</h2>
                </li>
            ...
            <div class="commentform">
                <br />
                <h3 style={{ color: 'black' }}> My Comments </h3>
                <ul class="commentlist">
                    {UserComments.length === 0 ? '아직 평가가 없어요' : UserComments && UserComments.map(UserComment =>
                        <li class="user_comment">
                            <h4 style={{ display: 'inline-block', color: '#636e72' }}>
                                영화ID : {UserComment.movie_id},
                                평점 : {UserComment.score},
                                내용 : {UserComment.body}
                            </h4>
                            <button style={{ float: 'right', marginRight: '1%', backgroundColor: 'black' }}>
                                <a href={'/movie/' + UserComment.movie_id}>수정</a>
                            </button>
                            <button style={{ float: 'right', marginRight: '1%', backgroundColor: 'black' }} onClick={() => DeleteComment(UserComment.movie_id)}>
                                <a href='#'>삭제</a>
                            </button>
                        </li>
                    )}
